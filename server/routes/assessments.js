...
export async function buildDashboardBundle(userId, result, profile) {
  ...
  return { result: usable, profile, learningPath, recommendations, history: await getHistory(userId) }
}
...
r.post(
  ...
  wrap(async (req, res) => {
    ...
    const prev = await getProfile(req.user.id)
    const profile = mergeProfile(prev, result)

    const assessmentId = await insertAssessment(req.user.id, {
      documentName,
      source,
      result,
      materialId: materialId ? Number(materialId) : null,
    })
    await upsertProfile(req.user.id, profile)

    const bundle = await buildDashboardBundle(req.user.id, result, profile)
    await saveRecommendationSnapshot(req.user.id, assessmentId, bundle.recommendations)
