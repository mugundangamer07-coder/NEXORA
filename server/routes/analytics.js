...
r.get(
  ...
  requireManager,
  wrap(async (_req, res) => {
    const learners = rowsToObjs(await client.execute("SELECT id, name, department FROM users WHERE role = 'learner'"))
    const profRows = rowsToObjs(await client.execute('SELECT user_id, competency, score FROM profile'))
    const assessCountRows = rowsToObjs(await client.execute('SELECT user_id, COUNT(*) c FROM assessments GROUP BY user_id'))
    const assessCounts = Object.fromEntries(assessCountRows.map((x) => [x.user_id, x.c]))
