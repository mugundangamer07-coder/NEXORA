...
import { insertMaterial, listMaterials, getMaterialById, UPLOAD_DIR } from '../db.js'
...
// Real file-type + size validation (defense in depth — never trust the browser alone).
const upload = multer({
...
r.post(
  ...
  wrap(async (req, res) => {
    ...
    const material = insertMaterial(req.user.id, {
...
r.get(
  ...
  wrap(async (req, res) => {
    ...
    res.json(listMaterials(req.user.id, { all: seeAll }))
  }),
)
...
r.get(
  ...
  requireAuth,
  wrap(async (req, res) => {
    const m = getMaterialById(Number(req.params.id))
